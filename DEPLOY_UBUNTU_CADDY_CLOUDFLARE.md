...
`docker-stack.ubuntu.yml` currently publishes Caddy with host-mode mapping `8080 -> 80`.
If you need a different host port, change `services.caddy.ports[0].published` to another integer (for example `9090`).
